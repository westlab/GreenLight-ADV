...
from greenlight.output_utils import get_mode, info
...
#        func_str = "def dy_from_str(t, y, d_matrix, t_span):\n"
        func_str = (
            "def dy_from_str(t, y, d_matrix, t_span):\n"
            "\tfrom greenlight.output_utils import output_row as __greenlight_output_row, info as __greenlight_info, get_mode as __greenlight_get_mode\n"
            "\tdy = np.zeros(y.shape)\n"
        )
        ...
        # 計算した行 (Time, states, inputs, auxiliary variables) をコンソールへ出力するコードを組み込む
        # d[0] は時間なので省略し、t を先頭に付加する
        if get_mode() == "show":
            func_str = func_str + (
                "\n\t# Dump current row as comma-separated values\n"
                "\tprint(','.join([str(val) for val in np.concatenate(([t], y, d[1:], a))]), flush=True)\n"
            )
...
#            + "+'%', end='',)\n"
#        )
        # 行データを出力する（MQTT またはコンソール）
        # d[0] は時間なので、t と d[1:]、状態 y、計算済み変数 a を連結して出力
        func_str += "\n\t__greenlight_output_row(np.concatenate(([t], y, d[1:], a)))\n"

        # 進捗表示は任意で残すことも可能（消す場合は以下を削除）
        func_str += (
            "\n\t# 進捗表示（show モードのとき info 非表示）\n"
            "\tprogress = '\\rRunning: ' + "
            "str(format(np.minimum(100, ((t - t_span[0]) / (t_span[1] - t_span[0])) * 100), '.2f')) + '%'\n"
            "\tif __greenlight_get_mode() != 'show':\n"
            "\t\t__greenlight_info(progress)\n"
        )
